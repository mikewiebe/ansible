...
from ansible.plugins.connection.jsonrpc import JsonRpcServer
...
class Connection(JsonRpcServer, _Connection):
    ...
    def _connect(self):
        ...
        try:
            ...
            raise AnsibleConnectionFailure('socket timeout connecting to device')
    ...
    def receive(self, command=None, prompts=None, answer=None):
        ...
        self._matched_prompt = None
        display.display(str(command), log_only=True)
    ...
    def exec_command(self, cmd):
        ...
        display.display('<<< %s' % str(obj), log_only=True)

        out = self._exec_rpc(obj)
        display.display('>>> %s' % str(out), log_only=True)
